...
You can use the exact same syntax to update an object. Note that Topaz assigned an `etag` property to the newly created object. Topaz utilizes an optimistic concurrency model for updates. In order to update an object, you need to specify the `etag` to ensure that the version you are modifying is the same object version you've retrieved. The `topaz directory set object` command does this automatically for you!
