...
You can also use the `policy` CLI to manually [build](/docs/command-line-interface/policy-cli/build), [tag](/docs/command-line-interface/policy-cli/tag) and [push](/docs/command-line-interface/policy-cli/push) the policy to a policy registry.
