...
title: Aserto SDKs - Single-Page Apps - Client installation and setup
description: Aserto SDKs - Single-Page Apps - Installing and using the Single-Page Apps client
---
...
A common problem for SPA front-ends is to conditionally display UI elements based on the set of permissions that the logged-in user possesses. For example, if a user only has "viewer" permissions,
the front-end typically wants to avoid displaying UI elements that allow users to edit or update resources.

We address this problem by defining the [**display state map**](/docs/authorizer-guide/display-state-map) pattern.

The Aserto JavaScript SPA SDK works together with other high-level language SDKs like the
[Express.js](/docs/software-development-kits/javascript/express#displaystatemap-middleware) to allow retrieving this **display state map** when the user logs in (and/or reload it). This in turn can be used to instruct the UI on whether to render specific UI elements that correspond to these routes / permissions.
