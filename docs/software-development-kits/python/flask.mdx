...
2. `register_display_state_map`: application middleware which implements and exposes an endpoint for returning the
   [display state map](/docs/authorizer-guide/display-state-map) for a service, based on its authorization policy.
...
- `authorizer_options` (_required_): An [`AuthorizerOptions`](/docs/software-development-kits/python/options) instance
  that describes the Authorizer service being used.
