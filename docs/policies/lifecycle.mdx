...
Policies are treated just like application code or infrastructure-as-code - they can be stored as code in a git repository, or as an OCIv2 image in a policy repository (like the Topaz Policy Container Registry or the [Open Policy Registry](https://www.openpolicyregistry.io/)).
...
{/* TODO: Instructions for updating configuration */}

In your `config.yaml` file, you'll now have to update the resource the authorizer will use to the newly published policy image.
