...
The `ds.check_relationship` built-in allows you to query the Topaz directory for a relationship between an object an a subject. Once we resolve the object and the subject, we can use the `ds.check_relationship` built-in to check if the object has a relationship with the subject.

```rego
allowed {
  user = ds.object(input.user.email)
  department = ds.object(input.resource.id)
  ds.check_relationship({
    "subject": user.id,
    "object": ds.object({
...
allowed {
  input.user.attributes.properties.department == "IT"
}
