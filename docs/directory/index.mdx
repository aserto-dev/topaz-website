...
The model defines **object types**. Each object type can have **relations** and **permissions**. Relations can be thought of as the "roles" that an object type supports. Permissions are the actions that can be performed on the object type, and are granted through relations.
...
types:
  ...
  resource:
    ...
    permissions:
      ...
      can_read: viewer | can_edit
```

* This manifest defines the `user` and `resource` types.
* The `user` type is simply used as a subject (target) for relationships and defined no relations or permissions of its own.
...
* The `resource` type has the  `can_delete` permission, which is granted to anyone with the `owner` relation to the resource. It also has the `can_write` permission, which is granted to anyone who has the `can_delete` permission on the file OR the `editor` relation to it. Finally, it has the `can_read` permission, which is granted to anyone with the `can_edit` permission OR the `viewer` relation.
